...
const pack = (scene) =>
  Buffer.from(deflateRawSync(Buffer.from(JSON.stringify(scene)))).toString('base64url');
...
async function load(scene) {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (
      await ev(
        `document.querySelectorAll('#metrics tr').length > 0 && !document.getElementById('boot')?.isConnected`,
      ).catch(() => false)
    )
      break;
  }
...
const click = async (selector) => {
  const r = JSON.parse(
    await ev(`JSON.stringify(document.querySelector('${selector}').getBoundingClientRect())`),
  );
...
const set = (id, value, event = 'input') =>
  ev(
    `{ const e = document.getElementById('${id}'); e.value = ${JSON.stringify(value)}; e.dispatchEvent(new Event('${event}', { bubbles: true })); }`,
  );
...
async function unnamed() {
  ...
  return nodes.filter(
    (n) => !n.ignored && roles.has(n.role?.value) && !(n.name?.value || '').trim(),
  ).length;
}
...
try {
  for (let i = 0; i < 50 && !ws; i++) {
    try {
      const page = (await (await fetch(`http://localhost:${PORT}/json`)).json()).find(
        (t) => t.type === 'page',
      );
  ...
  // a pointer over the views runs the hit test, which is the one path the checks below never touch
  {
    const r = JSON.parse(
      await ev(`JSON.stringify(document.getElementById('gl').getBoundingClientRect())`),
    );
    await send('Input.dispatchMouseEvent', {
      type: 'mouseMoved',
      x: r.x + r.width * 0.3,
  ...
  ok(
    (await ev(`[...document.querySelectorAll('label.f[for]')].filter((l) => l.control).length`)) ===
      15,
    'row labels reach their controls',
  );
  const c0 = await ev(CHROME);
  ok(
    c0.dim >= 4.5 && c0.hot >= 4.5 && c0.linkcr >= 4.5,
    'chrome contrast clears 4.5:1',
    `dim ${c0.dim.toFixed(2)} hot ${c0.hot.toFixed(2)} link ${c0.linkcr.toFixed(2)}`,
  );
  await ev(`document.getElementById('gl').focus()`);
  await key(']', 'BracketRight');
  ok(
    /^selected node 1,/.test(await live()),
    '] selects the first node and announces it',
    await live(),
  );
  ...
  ok(
    before !== (await ev(`document.getElementById('selinfo').textContent`)),
    'an arrow nudges the selected node',
  );
  ok(
    (await ev(`document.querySelectorAll('#swatches button[aria-label]').length`)) === 5,
    'swatches are named buttons',
  );
  ...
  ok(
    (await ev(`document.querySelectorAll('#metrics .sh[data-live]').length`)) > 0,
    'step shares appear during a run',
  );
  ok(
    (await ev(`document.getElementById('runinfo').textContent`)).startsWith('step '),
    'the step count shows',
    await ev(`document.getElementById('runinfo').textContent`),
  );
  ...
  await set('cvd', 'protan', 'change');
  await sleep(600);
  const c1 = await ev(CHROME);
  ok(c1.bg !== c0.bg && c1.link !== c0.link, 'the backdrop and accent follow the observer');
  ok(
    !(await ev(`document.getElementById('selsim').hidden`)),
    "the selection shows the observer's view",
  );
  await set('cvd', 'none', 'change');
  await sleep(600);
  ...
  ok(
    (await ev(`getComputedStyle(document.activeElement, '::after').content`)) !== 'none',
    'a hint shows on keyboard focus',
  );
  ok(
    (await ev(`document.getElementById('rate').getAttribute('aria-describedby')`)) === 'q-rate',
    'a control is described by its hint',
  );
  await ev(`document.querySelectorAll('.pcanvas')[0].focus()`);
  for (let i = 0; i < 30; i++) await key('ArrowUp', 'ArrowUp', 8);
  ok(
    /: 100$/.test(await live()),
    'a lightness knot goes past the band to the top of the axis',
    await live(),
  );
  ...
  for (let i = 0; i < 30; i++) await key('ArrowUp', 'ArrowUp', 8);
  ok(
    new RegExp(`: ${(h0 + 300) % 360}°$`).test(await live()),
    'a hue knot goes past the arc, round the circle',
    await live(),
  );
  ...
  ok(
    (await live()) === 'every hue allowed' &&
      (await ev(`document.getElementById('gl').getAttribute('aria-label')`)).includes(
        '5 control points',
      ),
    'Delete on the wheel resets the arc and keeps the nodes',
  );
