...
export function spaceMetric(k = params.space, mark = params.mark)         {
  ...
  return (p) => {
    const g = base(p);
    return [0, 1, 2].map((i) => [0, 1, 2].map((j) => t[i] * g[i][j] * t[j]))        ;
  };
}

function baseMetric(k        )         {
