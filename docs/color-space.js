 ...
 */
export function gamutFromICC(buf             , fallback = 'ICC profile')        {
  ...
  const xyz = (t        )       => {
    ...
    return [0, 1, 2].map((i) => d.getInt32(off + 8 + 4 * i) / 65536)        ;
  };
  const xy = (v      )                   => {
  ...
  if (chad !== undefined && str(chad, 4) === 'sf32') {
    const m = [0, 1, 2].map((r) => [0, 1, 2].map((c) => d.getInt32(chad + 8 + 4 * (3 * r + c)) / 65536))      ;
    const mi = inv3(m);
    un = (v      ) => apply(mi, v);
 ...
 */
export const visible = (xyz      , eps = 1e-9)          => {
...
const CAM = (() => {
  ...
  const achromatic = (c      ) => (2 * c[0] + c[1] + c[2] / 20 - 0.305) * Nbb;
  const Aw = achromatic(adapt(CHART_WHITE).map(nl)        );
...
export const de2000Form = ([L, a, b]      )       => {
  ...
  const R       = [[1, 0, 0], [0, k * c, sn], [0, -k * sn, c]];
  const x = RT / (2 * SC * SH);
  const M       = [[1 / (SL * SL), 0, 0], [0, 1 / (SC * SC), x], [0, x, 1 / (SH * SH)]];
  // R' M R
  return [0, 1, 2].map((i) => [0, 1, 2].map((j) =>
    [0, 1, 2].reduce((acc, u) => acc + [0, 1, 2].reduce(
      (t, v) => t + R[u][i] * M[u][v] * R[v][j], 0), 0)))        ;
};
 ...
 */
export const toSpace = (p      , k = params.space)       =>
  (SPACES[k].from === fromXYZ ? [p[0], p[1], p[2]] : SPACES[k].from(toXYZ(p)))        ;
export const fromSpace = (c      , k = params.space)       =>
  (SPACES[k].to === toXYZ ? [c[0], c[1], c[2]] : fromXYZ(SPACES[k].to(c)))        ;
...
function neutralScale(k        )         {
  if (NEUTRAL_SCALE[k] === undefined) {
    ...
    for (let i = 0; i < 128; i++) {
      ...
      len += form ? Math.sqrt(quad(form(a.map((v, j) => (v + b[j]) / 2)        ), d)) : Math.hypot(...d);
    }
...
const D_SPACE                                    = {
   ...
   */
  cam02: (xyz) => {
    const cone = apply(CAM.FWD, xyz.map((v) => v * 100)        );
    ...
    return mm(rows, CAM.FWD).map((r) => r.map((v) => v * 100))      ;
  },
...
export function spaceMetric(k = params.space)         {
  ...
  const gram = (col        , A             ) => {
    ...
    return [0, 1, 2].map((i) => [0, 1, 2].map((j) =>
      w * (col[i][0] * Ac[j][0] + col[i][1] * Ac[j][1] + col[i][2] * Ac[j][2])))        ;
  };
 ...
 */
export function geodesic(p      , q      , g         = EUCLIDEAN, n = 8, sweeps = 4)         {
  ...
  for (let s = 0; s < sweeps; s++)
    for (let i = 1; i < n; i++) {
      ...
      const y = apply(inv3(sum), [0, 1, 2].map((u) =>
        ...
        + gb[u][0] * b[0] + gb[u][1] * b[1] + gb[u][2] * b[2])        );
 ...
 */
export const lineDelta = (p      , q      , g         = EUCLIDEAN, n = 8)         =>
...
export function cvdMatrix(type     , severity = 1)           {
...
export const brettelTritan = (lin      )       => {
  const { h1, h2, n } = BRETTEL_TRITAN;
  const m = n[0] * lin[0] + n[1] * lin[1] + n[2] * lin[2] >= 0 ? h1 : h2;
  return [0, 1, 2].map((r) => m[3 * r] * lin[0] + m[3 * r + 1] * lin[1] + m[3 * r + 2] * lin[2])        ;
};
 ...
 */
export const tritanMix = (severity        ) => Math.min(1, Math.max(0, (severity - 0.8) / 0.2));
...
export function simulate(type     , severity = 1)       {
  ...
  return (p) => {
    ...
    const a = [0, 1, 2].map((r) => m[3 * r] * c[0] + m[3 * r + 1] * c[1] + m[3 * r + 2] * c[2])        ;
    if (w === 0) return fromLinear(a, 'srgb');
    const b = brettelTritan(c);
    return fromLinear(a.map((v, i) => v + w * (b[i] - v))        , 'srgb');
  };
}
 ...
 */
export function spline(controls        , perSpan = 24, closed = false)         {
...
function demo() {
  ...
  for (const g of ['srgb', 'display-p3', 'a98-rgb', 'rec2020']         ) {
    const bad = GAMUTS[g].p.filter(([x, y]) => !visible([x / y, 1, (1 - x - y) / y]));
    ok(bad.length === 0, `${g} primaries are all realisable`);
  }
  ok(GAMUTS['prophoto-rgb'].p.filter(([x, y]) => !visible([x / y, 1, (1 - x - y) / y])).length === 2,
  ...
  for (const [nm, m] of [['1', BRETTEL_TRITAN.h1], ['2', BRETTEL_TRITAN.h2]]                        ) {
